...
export default function BKLaporan() {
  ...
  const handleDownloadExcel = async () => {
    ...
    const totalCols = reportType === 'kasus' ? 12 : 9;
    ...
    let headers: string[] = [];
    if (reportType === 'kasus') {
      headers = ['NO', 'TANGGAL', 'KELAS', 'NAMA SISWA', 'KATEGORI KASUS', 'KRONOLOGI', 'PENANGANAN', 'KONSEKUENSI', 'CATATAN', 'GURU BK', 'WALI KELAS', 'STATUS'];
    } else {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-200 flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-1.5 bg-slate-50 p-1.5 rounded-2xl border border-slate-200">
          <input 
            type="date" 
            className="px-2 py-1.5 bg-transparent rounded-xl focus:ring-2 focus:ring-pink-500 outline-none text-xs font-semibold cursor-pointer"
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2">
                          ...
                          <div className="bg-blue-50/50 p-2 rounded-lg border border-blue-100">
                            <p className="text-[10px] font-bold text-blue-600 uppercase">Catatan</p>
                            <p className="text-sm text-slate-700">{d.catatan || '-'}</p>
                          </div>
                        </div>
                    ...
                    <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="mt-1 space-y-0.5">
                          ...
                          <p className="text-[10px] text-slate-500"><span className="font-bold">Catatan:</span> {d.catatan || '-'}</p>
