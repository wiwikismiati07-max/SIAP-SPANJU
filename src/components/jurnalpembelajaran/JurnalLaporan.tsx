...
export const JurnalLaporan: React.FC<JurnalLaporanProps> = ({ jurnalList, onRefresh, onEditJurnal }) => {
  ...
  const filteredJurnal = useMemo(() => {
    return jurnalList.filter(j => {
      ...
      // Kelas filter (supports exact, multi-class inclusion, and Inklusi)
      if (filterKelas !== 'semua') {
        if (filterKelas === 'Inklusi') {
          if (!j.kelas.toLowerCase().includes('inklusi')) return false;
        } else {
          const isDirect = j.kelas === filterKelas;
          const isPartOfMulti = j.kelas.split(',').map(s => s.trim()).includes(filterKelas);
          if (!isDirect && !isPartOfMulti) return false;
        }
      }
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-1">
              <div>
                ...
                >
                  <option value="semua">Semua Kelas</option>
                  <option value="Inklusi">⭐ Kelas Inklusi</option>
                          ...
                          return (
                            <tr key={j.id} className="hover:bg-slate-50/80 transition-colors">
                              ...
                              <td className="p-3 text-center">
                                {j.kelas.toLowerCase().includes('inklusi') ? (
                                  <span className="px-2 py-1 bg-indigo-50 text-indigo-800 font-extrabold rounded-lg border border-indigo-200 inline-flex items-center gap-1 text-[11px]">
                                    <Sparkles size={11} /> Inklusi
                                  </span>
                                ) : j.kelas.includes(',') ? (
                                  <span className="px-2 py-1 bg-amber-50 text-amber-800 font-extrabold rounded-lg border border-amber-200 inline-block text-[11px]" title={j.kelas}>
                                    Multi: {j.kelas}
                                  </span>
                                ) : (
