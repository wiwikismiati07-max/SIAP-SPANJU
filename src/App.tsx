          ...
          {activeSection === 'menu_aplikasi' && (
            ...
            >
              <div className="max-w-6xl mx-auto w-full space-y-10">
                ...
                {/* Add Menu Form (Admins only) */}
                {user?.role === 'full' && (
                  <motion.div 
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 space-y-4"
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <Plus className="text-pink-500" size={24} />
                      <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Form Tambah Menu Aplikasi</h3>
                    </div>
                    <form 
                      onSubmit={async (e) => {
                        e.preventDefault();
                        ...
                        if (supabase) {
                          try {
                            const { error } = await supabase.from('app_links').insert([newLink]);
                            if (error) throw error;
                          } catch (err) {
                    ...
                    >
                      <input 
                        ...
                        className="px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold focus:ring-2 focus:ring-pink-500 outline-none transition-all"
                        required
                      />
                      <input 
                        name="url"
                      ...
                      <button 
                        type="submit"
                        className="bg-slate-800 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-black transition-all shadow-lg"
                      >
                        Simpan Menu Baru
                      </button>
                    </form>
